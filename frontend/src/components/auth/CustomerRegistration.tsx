...
import { Eye, EyeOff, User, Mail, Phone, AlertCircle, CheckCircle } from 'lucide-react';
...
const CustomerRegistration: React.FC = () => {
  ...
  const { register, handleSubmit, formState: { errors: formErrors, isSubmitting }, setError, clearErrors, watch } = useForm<CustomerRegistrationForm>({
  ...
  const onSubmit = async (data: CustomerRegistrationForm) => {
    try {
      ...
      await registerCustomer({
        ...
        password: data.password, confirmPassword: data.password, role: 'customer',
        phone: data.phone, agreeToTerms: data.agreeToTerms, agreeToPrivacy: data.agreeToPrivacy,
      });
      setIsSuccess(true);
    } catch {
      if (errors?.length) {
        errors.forEach(err => {
          if (err.field) setError(err.field as keyof CustomerRegistrationForm, { type: 'server', message: err.message });
        });
      }
    }
  };
          ...
          <div className="glass rounded-3xl p-8 shadow-xl">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div className="space-y-3 pt-2">
                ...
                {formErrors.agreeToTerms && <p className="text-xs text-red-500">{formErrors.agreeToTerms.message}</p>}
                ...
                {formErrors.agreeToPrivacy && <p className="text-xs text-red-500">{formErrors.agreeToPrivacy.message}</p>}
              </div>
